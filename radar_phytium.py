...
基于 radar_debug.py (MP257版) 改写，仅修改串口设备路径
雷达协议和配置命令与开发板无关，应用层代码完全一致

飞腾派串口映射:
  - J1 DEBUG_UART1 (Pin7=TXD, Pin9=RXD) → /dev/ttyAMA1 (0x2800D000)
  - 40pin UART2 (Pin8=TXD, Pin10=RXD) → /dev/ttyAMA2 (0x2800E000) [已接LoRa]

用法:
  python3 radar_phytium.py                  默认: 配置BSD + 监控
  python3 radar_phytium.py -m               仅监控 (不发送配置命令)
  python3 radar_phytium.py -s               波特率扫描
  python3 radar_phytium.py -p /dev/ttyAMA2  指定串口
  python3 radar_phytium.py 115200           指定波特率
"""
...
def monitor(port: str, baud: int, do_config: bool = True, log_file: str = None):
    ...
    print(f"  飞腾派 PE2204 — DEBUG_UART1 (J1 Pin7/Pin9)")
    print("=" * 70)
    print_alarm_config()
...
def print_usage():
    print("用法:")
    print("  python3 radar_phytium.py                    默认: 配置BSD + 监控 + CSV记录")
    print("  python3 radar_phytium.py -m                 仅监控 (不发送配置命令)")
    print("  python3 radar_phytium.py -s                 波特率扫描")
    print("  python3 radar_phytium.py -p /dev/ttyAMA2    指定串口设备")
    print("  python3 radar_phytium.py -l FILE            指定CSV日志文件名")
    print("  python3 radar_phytium.py 115200             指定波特率")
    print()
    print("飞腾派串口映射:")
    print("  /dev/ttyAMA1  ← J1 DEBUG_UART1 (Pin7=TXD, Pin9=RXD)")
    print("  /dev/ttyAMA2  ← 40pin UART2 (Pin8=TXD, Pin10=RXD) [已接LoRa]")
    print()
    print("报警阈值 (修改脚本顶部常量):")
    for level, d, s, _ in ALARM_LEVELS:
